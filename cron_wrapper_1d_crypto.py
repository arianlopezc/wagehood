#!/usr/bin/env python3
"""
Cron Wrapper for 1-Day Crypto Analysis Trigger

This script manages the execution of the 1-day crypto analysis trigger with:
...
# Configuration
TIMEOUT_SECONDS = 90
SCRIPT_NAME = "trigger_1d_crypto_analysis.py"
LOCK_FILE = Path.home() / '.wagehood' / 'cron_1d_crypto.lock'
PID_FILE = Path.home() / '.wagehood' / 'cron_1d_crypto.pid'
LOG_FILE = Path.home() / '.wagehood' / 'cron_1d_crypto.log'
TIMEOUT_TRACKING_FILE = Path.home() / '.wagehood' / 'cron_1d_crypto_timeout_tracking.json'
...
class CronJobManager:
    ...
    async def send_infrastructure_alert(self, message: str):
        ...
        try:
            notification = NotificationMessage.create_service_notification(
                content=f"🚨 **1-Day Crypto Analysis Cron Job Issue**\n\n{message}\n\n⏰ Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    def run_with_timeout(self) -> bool:
        ...
        logger.info(f"Starting 1-day crypto analysis at {datetime.now()}")
        ...
        try:
            ...
            while process.poll() is None:
                ...
                if elapsed > TIMEOUT_SECONDS:
                    ...
                    asyncio.run(self.send_infrastructure_alert(
                        f"⚠️ TIMEOUT: 1-day crypto analysis exceeded {TIMEOUT_SECONDS}s timeout\n\n"
                        f"Timeout details:\n"
            ...
            # Log results
            if return_code == 0:
                logger.info(f"1-day crypto analysis completed successfully in {execution_time:.1f}s")
                # Reset timeout tracking on successful run
                self._reset_timeout_tracking()
            else:
                logger.error(f"1-day crypto analysis failed with code {return_code} after {execution_time:.1f}s")
...
def main():
    ...
    try:
        ...
        logger.info(f"Starting 1-day crypto analysis cron job at {datetime.now()}")
        ...
        if success:
            logger.info("1-day crypto analysis completed successfully")
            return 0
        else:
            logger.error("1-day crypto analysis failed")
            return 1
