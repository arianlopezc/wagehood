...
class MACDRSIAnalyzer:
    ...
    def _validate_inputs(
        ...
        min_required_periods = max(macd_slow, rsi_period) + 10  # Buffer
        
        if timeframe == "1h":
            min_days = max(5, min_required_periods // 24 + 1)
        else:  # 1d
            min_days = max(min_required_periods, 30)
    ...
    async def _fetch_market_data(
        ...
        try:
            ...
            macd_slow = self.macd_rsi_strategy.parameters.get("macd_slow", 26)
            rsi_period = self.macd_rsi_strategy.parameters.get("rsi_period", 14)
            min_required = max(macd_slow, rsi_period) + 10
    ...
    async def _calculate_indicators(self, market_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            indicators = {
                "macd": {
                    "macd": {
                        "macd": macd_line.tolist() if hasattr(macd_line, 'tolist') else macd_line,
                        "signal": signal_line.tolist() if hasattr(signal_line, 'tolist') else signal_line,
                        "histogram": histogram.tolist() if hasattr(histogram, 'tolist') else histogram
                    }
                },
                "rsi": {
                    f"rsi_{rsi_period}": rsi_values.tolist() if hasattr(rsi_values, 'tolist') else rsi_values
                }
            }
            
            self.logger.debug(f"Calculated MACD+RSI indicators for {market_data.get('symbol', 'unknown')}")
            return indicators
