...
class RSITrendFollowing(TradingStrategy):
    ...
    def generate_signals(self, data: MarketData, indicators: Dict[str, Any]) -> List[Signal]:
        ...
        try:
            ...
            for i in range(self.parameters['trend_confirmation_periods'], len(rsi_values)):
                ...
                if self.parameters['divergence_detection']:
                    divergence_signal = self._check_divergence_signal(data, rsi_values, i)
    ...
    def _check_uptrend_signal(self, data: MarketData, rsi_values: np.ndarray, index: int) -> Signal:
        ...
        pullback_entry = (
            ...
            (self.parameters['uptrend_pullback_low'] <= current_rsi <= self.parameters['uptrend_pullback_high']) and
            # RSI starting to turn up
            (current_rsi > previous_rsi or current_rsi > self.parameters['uptrend_pullback_low'])
        )
    ...
    def _check_downtrend_signal(self, data: MarketData, rsi_values: np.ndarray, index: int) -> Signal:
        ...
        rally_entry = (
            ...
            (self.parameters['downtrend_pullback_low'] <= current_rsi <= self.parameters['downtrend_pullback_high']) and
            # RSI starting to turn down
            (current_rsi < previous_rsi or current_rsi < self.parameters['downtrend_pullback_high'])
        )
        ...
        # Calculate confidence
        confidence_factors = self._calculate_downtrend_confidence(data, rsi_values, index)
        confidence = self.calculate_confidence(confidence_factors)
    ...
    def _calculate_uptrend_confidence(self, data: MarketData, rsi_values: np.ndarray, index: int) -> Dict[str, float]:
        ...
        rsi_range = self.parameters['uptrend_pullback_high'] - self.parameters['uptrend_pullback_low']
        rsi_position = 1.0 - ((current_rsi - self.parameters['uptrend_pullback_low']) / rsi_range)
    ...
    def _calculate_downtrend_confidence(self, data: MarketData, rsi_values: np.ndarray, index: int) -> Dict[str, float]:
        ...
        rsi_range = self.parameters['downtrend_pullback_high'] - self.parameters['downtrend_pullback_low']
        rsi_position = (current_rsi - self.parameters['downtrend_pullback_low']) / rsi_range
        confidence_factors['rsi_position'] = max(0.0, min(1.0, rsi_position))
    ...
    def _calculate_trend_strength(self, rsi_values: np.ndarray, index: int, trend_type: str) -> float:
    ...
    def _calculate_price_momentum(self, data: MarketData, index: int, trend_type: str) -> float:
    ...
    def _calculate_rsi_momentum(self, rsi_values: np.ndarray, index: int, trend_type: str) -> float:
    ...
    def _check_divergence_signal(self, data: MarketData, rsi_values: np.ndarray, index: int) -> Signal:
    ...
    def _detect_bullish_divergence(self, prices: np.ndarray, rsi_values: np.ndarray) -> bool:
    ...
    def _detect_bearish_divergence(self, prices: np.ndarray, rsi_values: np.ndarray) -> bool:
    ...
    def _create_divergence_signal(self, data: MarketData, index: int, direction: str) -> Signal:
