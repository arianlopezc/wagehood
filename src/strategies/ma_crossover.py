...
class MovingAverageCrossover(TradingStrategy):
    ...
    def generate_signals(self, data: MarketData, indicators: Dict[str, Any]) -> List[Signal]:
        ...
        try:
            ...
            short_ema = indicators.get('ema', {}).get(f'ema_{self.parameters["short_period"]}')
            long_ema = indicators.get('ema', {}).get(f'ema_{self.parameters["long_period"]}')
    ...
    def _create_golden_cross_signal(self, data: MarketData, index: int, short_ema: float, 
                                   long_ema: float, volume_data: np.ndarray = None, 
        ...
        confidence_factors['ema_separation'] = min(1.0, ema_separation * 10)  # Scale to 0-1
    ...
    def _create_death_cross_signal(self, data: MarketData, index: int, short_ema: float, 
                                  long_ema: float, volume_data: np.ndarray = None, 
                                  avg_volume: float = None) -> Signal:
    ...
    def _calculate_trend_strength(self, data: MarketData, index: int, direction: str) -> float:
        ...
        if direction == 'bullish':
            ...
            up_periods = sum(1 for i in range(1, len(recent_closes)) 
