...
class SRBreakoutAnalyzer:
    ...
    async def analyze_symbol(
        ...
        try:
            ...
            self.logger.info(
                f"Generated {len(signals)} SR Breakout signals for {symbol}"
            )
            return signals
    ...
    def _validate_inputs(
        ...
        validation_result = DataRequirementsCalculator.validate_date_range(
            ...
            end_date=end_date,
            strategy_type="sr",
            strategy_parameters=self.sr_strategy.parameters,
            timeframe=timeframe,
        )
    ...
    async def _calculate_indicators(
        ...
        try:
            ...
            sr_levels = calculate_support_resistance(
                close_prices, lookback=lookback_periods, min_touches=min_touches
            )

            indicators = {"support_resistance": sr_levels}

            self.logger.debug(
                f"Calculated S/R levels for {market_data.get('symbol', 'unknown')}"
            )
            return indicators
