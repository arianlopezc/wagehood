...
STRATEGY_METADATA = {
    ...
    'macd_rsi': {
        ...
        'best_conditions': ['Trending markets', 'Volatile markets with clear direction'],
...
def get_strategy_class(strategy_name: str) -> type:
    ...
    if strategy_name not in STRATEGY_REGISTRY:
        raise KeyError(f"Strategy '{strategy_name}' not found. Available strategies: {list(STRATEGY_REGISTRY.keys())}")
...
def get_beginner_strategies() -> list:
    ...
    """
    return [
        name for name, metadata in STRATEGY_METADATA.items()
