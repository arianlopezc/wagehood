...
class RSITrendAnalyzer:
    ...
    async def analyze_symbol(
        ...
        try:
            ...
            validation_result = self._validate_inputs_cached(
    ...
    def _validate_inputs_cached(
        ...
        if (
            ...
            and (current_time - self._validation_cache_time).seconds < 300
            and hasattr(self, "_last_cache_key")
    ...
    async def _fetch_market_data(
        ...
        try:
            ...
            # Validate minimum data requirements
            min_periods = self.rsi_strategy.parameters.get(
    ...
    async def _calculate_indicators(
        ...
        try:
            ...
            rsi_21 = (
                calculate_rsi(close_prices, rsi_main_period)
                if rsi_main_period != rsi_period
                else rsi_14
            )
            ...
            indicators = {
                "rsi": {
                    f"rsi_{rsi_period}": (
                        rsi_14.tolist() if hasattr(rsi_14, "tolist") else rsi_14
                    ),
                    f"rsi_{rsi_main_period}": (
                        rsi_21.tolist() if hasattr(rsi_21, "tolist") else rsi_21
                    ),
                }
            }

            self.logger.debug(
                f"Calculated RSI indicators for {market_data.get('symbol', 'unknown')}"
            )
            return indicators
