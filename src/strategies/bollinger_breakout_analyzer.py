...
class BollingerBreakoutAnalyzer:
    ...
    async def analyze_symbol(
        ...
        try:
            ...
            self.logger.info(f"Generated {len(signals)} Bollinger Breakout signals for {symbol}")
            return signals
    ...
    def _validate_inputs(
        ...
        min_periods = self.bollinger_strategy.parameters.get("consolidation_periods", 10)
        bb_period = self.bollinger_strategy.parameters.get("bb_period", 20)
        min_required_periods = max(min_periods, bb_period) + 5  # Buffer
        
        if timeframe == "1h":
            min_days = max(3, min_required_periods // 24 + 1)
        else:  # 1d
            min_days = max(min_required_periods, 25)
    ...
    async def _fetch_market_data(
        ...
        try:
            ...
            min_periods = self.bollinger_strategy.parameters.get("consolidation_periods", 10)
            bb_period = self.bollinger_strategy.parameters.get("bb_period", 20)
            min_required = max(min_periods, bb_period) + 5
            ...
            indicators = {
                "bollinger": {
                    f"bollinger_{bb_period}": {
                        "upper": upper_band.tolist() if hasattr(upper_band, 'tolist') else upper_band,
                        "middle": middle_band.tolist() if hasattr(middle_band, 'tolist') else middle_band,
                        "lower": lower_band.tolist() if hasattr(lower_band, 'tolist') else lower_band
                    }
                }
            }
            
            self.logger.debug(f"Calculated Bollinger Bands for {market_data.get('symbol', 'unknown')}")
            return indicators
