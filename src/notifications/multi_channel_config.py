...
class StrategyChannelConfig:
    ...
    emoji: str = "📊"
    allowed_timeframes: List[str] = field(default_factory=lambda: ["1d"])  # Supported timeframes
...
class MultiChannelNotificationConfig:
    ...
    def _build_strategy_configs(cls) -> Dict[str, StrategyChannelConfig]:
        ...
        for strategy_key, strategy_info in STRATEGY_CONFIGS.items():
            ...
            allowed_timeframes, trading_profile = cls._get_strategy_timeframes_and_profile(
            ...
            if webhook_url and strategy_enabled:
                ...
                logger.info(f"Configured Discord channel for {strategy_info['name']} ({trading_profile}, {allowed_timeframes})")
            else:
    ...
    @classmethod
    def _get_strategy_timeframes_and_profile(cls, strategy_key: str, mapping: Dict[str, List[str]]) -> tuple:
