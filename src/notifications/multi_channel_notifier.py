...
class MultiChannelDiscordNotifier:
    ...
    def __init__(self, config: MultiChannelNotificationConfig):
        ...
        if self.config.fallback_webhook_url:
            ...
            self.fallback_rate_limiter = NotificationRateLimiter(
                self.config.fallback_rate_limit
            )
        ...
        for strategy_key, strategy_config in self.config.strategy_channels.items():
            if strategy_config.enabled and strategy_config.webhook_url:
                ...
                rate_limiter = NotificationRateLimiter(
                    strategy_config.max_notifications_per_hour
                )
                self.strategy_rate_limiters[strategy_key] = rate_limiter

                logger.info(
                    f"Initialized Discord notifier for {strategy_config.strategy_name}"
                )

    def send_strategy_notification(
        self, strategy_name: str, signal_data: Dict[str, Any]
    ) -> bool:
        """
        ...
        try:
            ...
            if not strategy_config:
                ...
                return self._send_fallback_notification(
                    signal_data, f"Unknown strategy: {strategy_name}"
                )
            ...
            if not notifier or not rate_limiter:
                logger.error(
                    f"Notifier or rate limiter not found for strategy: {strategy_name}"
                )
                return self._send_fallback_notification(
                    signal_data, f"Configuration error: {strategy_name}"
                )
            ...
            logger.error(
                f"Error sending strategy notification for {strategy_name}: {e}"
            )
            return False

    def _send_fallback_notification(
        self, signal_data: Dict[str, Any], reason: str
    ) -> bool:
        """
    ...
    def _create_strategy_embed(
        self, strategy_config: StrategyChannelConfig, signal_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        embed = {
            ...
            "fields": [
                ...
                {"name": "⏰ Timeframe", "value": timeframe.upper(), "inline": True},
            ],
            "footer": {
        ...
        if details:
            detail_text = self._format_strategy_details(
                strategy_config.strategy_name, details
            )
            if detail_text:
                embed["fields"].append(
                    {"name": "📋 Signal Details", "value": detail_text, "inline": False}
                )

        return embed

    def _format_strategy_details(
        self, strategy_name: str, details: Dict[str, Any]
    ) -> str:
        """
        ...
        if "macd" in strategy_name.lower() and "rsi" in strategy_name.lower():
            ...
            "resistance" in strategy_name.lower() or "support" in strategy_name.lower()
        ):
    ...
    def _create_fallback_embed(
        ...
        embed = {
            ...
            "fields": [
                ...
                {"name": "⚠️ Fallback Reason", "value": reason, "inline": False},
            ],
            "footer": {
