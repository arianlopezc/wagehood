"""Analysis service for signal analysis and strategy comparison."""
...
from ..core.models import TimeFrame, StrategyStatus, SignalAnalysisResult
from ..jobs.distributed import DistributedJobManager
from ..jobs.models import JobType
from ..backtest.engine import SignalAnalysisEngine, SignalAnalysisConfig
...
class AnalysisService:
    ...
    def __init__(self):
        ...
        logger.info("AnalysisService initialized with signal analysis support")
    ...
    async def calculate_indicators(
        ...
        try:
            logger.info(
                f"Calculating {len(indicators)} indicators for {symbol} ({timeframe})"
            )
    ...
    async def optimize_strategy(
        ...
        try:
            ...
            logger.info(
                f"Optimization completed with {len(results['results'])} parameter combinations"
            )
    ...
    async def optimize_strategy_async(
        ...
        # Start async task
        asyncio.create_task(
            self._optimize_strategy_async_task(
                job_id,
                symbol,
                timeframe,
                strategy,
    ...
    async def compare_strategies(
        ...
        try:
            logger.info(
                f"Comparing {len(strategies)} strategies for {symbol} ({timeframe})"
            )
    ...
    async def get_strategy_rankings(
        ...
        try:
            ...
            rankings = self.evaluator.get_strategy_rankings(
                metric=metric, symbol=symbol, timeframe=timeframe, limit=limit
            )
    ...
    async def get_best_strategy(
        self, symbol: str, timeframe: TimeFrame, metric: str
        ...
        try:
            ...
            best_strategy = self.evaluator.get_best_strategy(
                symbol=symbol, timeframe=timeframe, metric=metric
            )
    ...
    async def get_correlation_analysis(
        self, symbols: List[str], timeframe: TimeFrame, lookback_days: int = 252
    ) -> Dict[str, Any]:
        """
    ...
    async def get_risk_metrics(
        ...
        try:
            logger.info(
                f"Calculating risk metrics for {strategy} on {symbol} ({timeframe})"
            )
    ...
    async def get_performance_attribution(self, backtest_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"Performing performance attribution for backtest {backtest_id}"
            )
    ...
    async def analyze_strategy(self, strategy, market_data):
        ...
        try:
            ...
            strategy_params = (
                strategy.get_parameters() if hasattr(strategy, "get_parameters") else {}
            )
    ...
    async def analyze_strategy_historical(
        ...
        try:
            ...
            logger.info(
                f"Created distributed historical analysis job {job_id} for {symbol} {strategy}"
            )
            return job_id
    ...
    async def _process_historical_analysis_job(
        ...
        try:
            ...
            strategy_instance = (
                strategy_class(parameters) if parameters else strategy_class()
            )
            ...
            logger.info(
                f"Completed historical analysis job {job_id}: {len(signal_data)} signals generated"
            )
    ...
    async def _create_legacy_historical_job(
        ...
        # Start async task
        asyncio.create_task(
            self._process_historical_analysis_job(
    ...
    async def _process_distributed_historical_job(self, job_id: str):
        ...
        try:
            ...
            if result:
                ...
                await self.distributed_job_manager.update_job_status(
                    job_id, JobStatus.FAILED, error_message="Job execution failed"
                )
    ...
    def run_signal_analysis(self, strategy, market_data, config=None):
        """Run signal analysis for a strategy with market data"""
        try:
            engine = SignalAnalysisEngine(config or SignalAnalysisConfig())
            return engine.run_signal_analysis(strategy, market_data)
        except Exception as e:
            logger.error(f"Error running signal analysis: {e}")
            raise

    def analyze_strategy_signals(self, strategy, market_data):
        """Analyze strategy signals for quality and characteristics"""
        try:
            return self.run_signal_analysis(strategy, market_data)
        except Exception as e:
            logger.error(f"Error analyzing strategy signals: {e}")
            raise

    def compare_signal_quality(self, strategies, market_data):
        """Compare signal quality across multiple strategies"""
        try:
            results = {}
            for strategy in strategies:
                results[strategy.name] = self.analyze_strategy_signals(
                    strategy, market_data
                )
            return results
        except Exception as e:
            logger.error(f"Error comparing signal quality: {e}")
            raise
