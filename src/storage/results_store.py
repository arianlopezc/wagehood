...
from ..core.models import BacktestResult, PerformanceMetrics, Trade, Signal
...
class ResultsStore:
    ...
    def __init__(self, storage_path: str = "backtest_results", storage_type: str = "json", 
    ...
    def _init_database(self):
        ...
        # Create indexes
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_strategy_symbol ON backtest_results (strategy_name, symbol)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_created_at ON backtest_results (created_at)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_performance_metrics ON performance_metrics (backtest_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_trades ON trades (backtest_id)')
        
        conn.commit()
        conn.close()
    
    def save_result(self, result: BacktestResult, metadata: Dict[str, Any] = None) -> str:
        """
    ...
    def load_result(self, result_id: str) -> Optional[BacktestResult]:
        """
        ...
        try:
            ...
            if result is not None and self.enable_cache:
                cache_key = cache_manager.cache_key_hash("result", result_id)
                cache_manager.set("results", cache_key, result, self._cache_ttl_settings['results'])
    ...
    def list_results(self, 
        ...
        if self.enable_cache and strategy_name is None and symbol is None and date_range is None:
        ...
        try:
            ...
            if self.enable_cache and strategy_name is None and symbol is None and date_range is None:
                cache_key = cache_manager.cache_key_hash("list_results", "all")
                cache_manager.set("results", cache_key, results, self._cache_ttl_settings['summaries'])
    ...
    def get_performance_summary(self, 
        ...
        if self.enable_cache and strategy_name is None and symbol is None:
            cache_key = cache_manager.cache_key_hash("performance_summary", "all")
            cache_manager.set("results", cache_key, summary, self._cache_ttl_settings['performance'])
    ...
    def _save_json(self, result: BacktestResult, metadata: Dict[str, Any] = None) -> str:
        """Save result as JSON"""
    ...
    def _load_json(self, filename: str) -> Optional[BacktestResult]:
    ...
    def _list_json(self, strategy_name: str, symbol: str, date_range: Tuple[datetime, datetime]) -> List[Dict[str, Any]]:
        """List JSON results"""
        results = []
        ...
        for filepath in self.storage_path.glob("*.json"):
            try:
                ...
                summary = {
                    ...
                    'total_return_pct': result_dict.get('performance_metrics', {}).get('total_return_pct', 0),
                    'sharpe_ratio': result_dict.get('performance_metrics', {}).get('sharpe_ratio', 0),
                    'win_rate': result_dict.get('performance_metrics', {}).get('win_rate', 0),
                    'total_trades': result_dict.get('performance_metrics', {}).get('total_trades', 0)
                }
    ...
    def _save_pickle(self, result: BacktestResult, metadata: Dict[str, Any] = None) -> str:
    ...
    def _load_pickle(self, filename: str) -> Optional[BacktestResult]:
    ...
    def _list_pickle(self, strategy_name: str, symbol: str, date_range: Tuple[datetime, datetime]) -> List[Dict[str, Any]]:
        """List pickle results"""
        results = []
    ...
    def _save_sqlite(self, result: BacktestResult, metadata: Dict[str, Any] = None) -> str:
        ...
        try:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                backtest_id, metrics.total_trades, metrics.winning_trades, metrics.losing_trades,
                metrics.win_rate, metrics.total_pnl, metrics.total_return_pct, metrics.max_drawdown,
                metrics.max_drawdown_pct, metrics.sharpe_ratio, metrics.sortino_ratio, metrics.profit_factor,
                metrics.avg_win, metrics.avg_loss, metrics.largest_win, metrics.largest_loss,
                metrics.avg_trade_duration_hours, metrics.max_consecutive_wins, metrics.max_consecutive_losses
            ))
            ...
            for trade in result.trades:
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    backtest_id, trade.trade_id, trade.entry_time.isoformat(),
                    trade.exit_time.isoformat() if trade.exit_time else None,
                    trade.symbol, trade.quantity, trade.entry_price, trade.exit_price,
                    trade.pnl, trade.commission, trade.strategy_name,
    ...
    def _load_sqlite(self, result_id: str) -> Optional[BacktestResult]:
        ...
        try:
            # Get main result
            cursor.execute('SELECT result_data FROM backtest_results WHERE id = ?', (result_id,))
            row = cursor.fetchone()
    ...
    def _list_sqlite(self, strategy_name: str, symbol: str, date_range: Tuple[datetime, datetime]) -> List[Dict[str, Any]]:
        """List SQLite results"""
        conn = sqlite3.connect(str(self.storage_path))
        cursor = conn.cursor()
    ...
    def _delete_sqlite(self, result_id: str) -> bool:
        ...
        try:
            ...
            cursor.execute('DELETE FROM performance_metrics WHERE backtest_id = ?', (result_id,))
    ...
    def _result_to_dict(self, result: BacktestResult) -> Dict[str, Any]:
        ...
        return {
            ...
            'initial_capital': result.initial_capital,
            'final_capital': result.final_capital,
            'trades': [self._trade_to_dict(trade) for trade in result.trades],
            'equity_curve': result.equity_curve,
            'performance_metrics': self._metrics_to_dict(result.performance_metrics),
            'signals': [self._signal_to_dict(signal) for signal in result.signals]
        }
    
    def _dict_to_result(self, result_dict: Dict[str, Any]) -> BacktestResult:
        """Convert dictionary to BacktestResult"""
        return BacktestResult(
            ...
            trades=[self._dict_to_trade(trade_dict) for trade_dict in result_dict['trades']],
            equity_curve=result_dict['equity_curve'],
            performance_metrics=self._dict_to_metrics(result_dict['performance_metrics']),
            signals=[self._dict_to_signal(signal_dict) for signal_dict in result_dict['signals']]
        )
    ...
    def _dict_to_trade(self, trade_dict: Dict[str, Any]) -> Trade:
        ...
        return Trade(
            ...
            signal_metadata=trade_dict['signal_metadata']
        )
    ...
    def _invalidate_cache_for_result(self, result: BacktestResult, result_id: str) -> None:
        """
        ...
        try:
            ...
            cache_manager.delete("results", cache_manager.cache_key_hash("list_results", "all"))
            
            # Invalidate performance summary cache
            cache_manager.delete("results", cache_manager.cache_key_hash("performance_summary", "all"))
            
            # Invalidate specific result cache if it exists
            cache_manager.delete("results", cache_manager.cache_key_hash("result", result_id))
    ...
    def _invalidate_cache_for_deletion(self, result_id: str) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to invalidate cache for deleted result {result_id}: {e}")
