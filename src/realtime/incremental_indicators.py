...
class IncrementalIndicatorCalculator:
    ...
    def __init__(self):
        ...
        self._states: Dict[str, Dict[str, IndicatorState]] = (
            {}
        )  # {symbol: {indicator: state}}
        self._initialized: Dict[str, set] = (
            {}
        )  # Track which indicators are initialized per symbol
    ...
    def calculate_sma_incremental(
        self, symbol: str, price: float, period: int = MA_FAST
    ) -> Optional[float]:
        """
        ...
        try:
            ...
            if state_key not in self._states[symbol]:
                ...
                self._states[symbol][state_key] = EMAState(
                    last_updated=datetime.now(), period=period, value=price, alpha=alpha
                )
                return price
    ...
    def calculate_rsi_incremental(
        self, symbol: str, price: float, period: int = RSI_PERIOD
    ) -> Optional[float]:
        """
    ...
    def calculate_macd_incremental(
        ...
        try:
            ...
            state_key = self._get_state_key(
                symbol, "macd", fast=fast, slow=slow, signal=signal
            )
            ...
            state.signal_line = (state.macd_line * signal_alpha) + (
                state.signal_line * (1 - signal_alpha)
            )
    ...
    def calculate_bollinger_bands_incremental(
        ...
        try:
            self._ensure_symbol_initialized(symbol)
            state_key = self._get_state_key(
                symbol, "bb", period=period, std_dev=std_dev
            )
    ...
    def get_indicator_value(
        self, symbol: str, indicator: str, **params
    ) -> Optional[float]:
        """
        ...
        try:
            summary = {
                ...
                "total_indicators": sum(
                    len(indicators) for indicators in self._states.values()
                ),
    ...
    def generate_trading_signals(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if bb_state:
                ...
                signals["bb_squeeze"] = (
                    bb_state.upper_band - bb_state.lower_band
                ) / bb_state.middle_band < 0.1
