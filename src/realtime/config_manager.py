"""
Configuration Manager for Real-time Signal Detection

This module provides centralized configuration management for the real-time
signal detection system, including watchlist management, signal detection parameters,
...
class SignalProfile(Enum):
    """Signal profile that determines timeframe and update frequency settings."""

    HIGH_FREQUENCY = "high_frequency"  # 1m-15m signals
    MEDIUM_FREQUENCY = "medium_frequency"  # 15m-4h signals
    LOW_FREQUENCY = "low_frequency"  # 4h-1d signals
...
class TimeframeConfig:
    ...
    def validate(self) -> bool:
        ...
        return (
            self.timeframe in valid_timeframes
            and self.update_interval_seconds > 0
            and self.lookback_periods > 0
            and self.min_data_points > 0
        )


@dataclass
class StrategyTimeframeMatrix:
    """Maps strategies to their appropriate timeframes and signal profiles."""
...
class SymbolStrategyConfig:
    ...
    signal_profile: SignalProfile = SignalProfile.MEDIUM_FREQUENCY
...
class StrategyConfig:
    """Configuration for a signal detection strategy."""
...
class ConfigManager:
    """
    Centralized configuration manager for real-time signal detection.

    This class manages all configuration aspects including:
    - Watchlist symbols and their settings
    - Technical indicator configurations
    - Signal detection strategy configurations
    ...
    def get_default_timeframe_configs(
        self,
    ) -> Dict[SignalProfile, List[TimeframeConfig]]:
        """Get default timeframe configurations for each signal profile."""
        return {
            SignalProfile.HIGH_FREQUENCY: [
                TimeframeConfig(
                    timeframe="1m",
            ...
            SignalProfile.MEDIUM_FREQUENCY: [
                TimeframeConfig(
                    timeframe="5m",
                    update_interval_seconds=5,
                    lookback_periods=300,
    ...
    def get_default_strategy_matrix(self) -> List[StrategyTimeframeMatrix]:
        ...
        return [
            StrategyTimeframeMatrix(
                ...
                optimal_timeframe="1d",  # Better for swing trading
                signal_profile=SignalProfile.MEDIUM_FREQUENCY,
            ...
            StrategyTimeframeMatrix(
                ...
                signal_profile=SignalProfile.LOW_FREQUENCY,
                min_bars_required=250,
            ),
            StrategyTimeframeMatrix(
                strategy_name="rsi_trend_strategy",
                supported_timeframes=["5m", "15m", "1h", "4h"],
                optimal_timeframe="1h",  # Good for day trading
                signal_profile=SignalProfile.MEDIUM_FREQUENCY,
            ...
            StrategyTimeframeMatrix(
                ...
                optimal_timeframe="1h",  # Better for day trading alerts
                signal_profile=SignalProfile.HIGH_FREQUENCY,
                min_bars_required=100,
            ),
            StrategyTimeframeMatrix(
                strategy_name="sr_breakout_strategy",
                supported_timeframes=["5m", "15m", "1h", "4h", "1d"],
                optimal_timeframe="1d",  # Better for swing trading alerts
                signal_profile=SignalProfile.MEDIUM_FREQUENCY,
                min_bars_required=200,
            ),
        ]
    ...
    def _get_default_system_config(self) -> SystemConfig:
        ...
        return SystemConfig(
            max_concurrent_signals=int(os.environ.get("MAX_CONCURRENT_SIGNALS", 100)),
            batch_signal_size=int(os.environ.get("BATCH_SIGNAL_SIZE", 10)),
            data_update_interval_seconds=int(os.environ.get("DATA_UPDATE_INTERVAL", 1)),
            signal_workers=int(os.environ.get("SIGNAL_WORKERS", 4)),
            redis_streams_maxlen=int(os.environ.get("REDIS_STREAMS_MAXLEN", 10000)),
            enable_monitoring=os.environ.get("ENABLE_MONITORING", "true").lower()
            == "true",
            enable_discord_notifications=os.environ.get(
        ...
        try:
            ...
            if cached_data:
                ...
                for asset_data in cached_data:
                    ...
                    if "signal_profile" not in asset_data:
                        asset_data["signal_profile"] = SignalProfile.MEDIUM_FREQUENCY
                    elif isinstance(asset_data["signal_profile"], str):
                        asset_data["signal_profile"] = SignalProfile(
                            asset_data["signal_profile"]
                        )
        ...
        try:
            ...
            for asset in assets:
                if asset.symbol not in SUPPORTED_SYMBOLS:
                    logger.warning(
                        f"Symbol {asset.symbol} not in supported symbols list"
                    )
            ...
            for asset in assets:
                ...
                if isinstance(data.get("signal_profile"), SignalProfile):
                    data["signal_profile"] = data["signal_profile"].value
                data["last_updated"] = datetime.now().isoformat()
                asset_data.append(data)

            # Store in cache
            success = cache_manager.set(
                self._config_namespace, "watchlist", asset_data, self._cache_ttl
            )
    ...
    def add_symbol(
        ...
        signal_profile: SignalProfile = None,
    ) -> bool:
        """
        ...
        Args:
            ...
            signal_profile: Signal profile for this symbol
        ...
        try:
            ...
            new_asset = AssetConfig(
                ...
                or os.environ.get("DATA_PROVIDER", "alpaca"),
                timeframes=timeframes or ["1m", "5m", "1h", "1d"],
                priority=priority,
                enabled_strategies=enabled_strategies
                or ["macd_rsi_strategy", "rsi_trend_strategy"],
                signal_profile=signal_profile or SignalProfile.MEDIUM_FREQUENCY,
            )
    ...
    def remove_symbol(self, symbol: str) -> bool:
        ...
        try:
            current_watchlist = self.get_watchlist()
            updated_watchlist = [
                asset for asset in current_watchlist if asset.symbol != symbol
            ]
    ...
    def get_indicator_configs(self) -> List[IndicatorConfig]:
        ...
        try:
            ...
            if cached_data:
                return [
                    IndicatorConfig(**indicator_data) for indicator_data in cached_data
                ]
    ...
    def update_indicator_configs(self, indicators: List[IndicatorConfig]) -> bool:
        ...
        try:
            ...
            success = cache_manager.set(
                self._config_namespace, "indicators", indicator_data, self._cache_ttl
            )
        ...
        try:
            ...
            if cached_data:
                ...
                for strategy_data in cached_data:
                    ...
                    if "signal_profile" not in strategy_data:
                        strategy_data["signal_profile"] = SignalProfile.MEDIUM_FREQUENCY
                    elif isinstance(strategy_data["signal_profile"], str):
                        strategy_data["signal_profile"] = SignalProfile(
                            strategy_data["signal_profile"]
                        )
    ...
    def update_strategy_configs(self, strategies: List[StrategyConfig]) -> bool:
        ...
        try:
            ...
            success = cache_manager.set(
                self._config_namespace, "strategies", strategy_data, self._cache_ttl
            )
    ...
    def get_timeframe_configs(self) -> Dict[SignalProfile, List[TimeframeConfig]]:
        """
        Get current timeframe configurations.

        Returns:
            Dictionary mapping signal profiles to their timeframe configs
        """
        try:
            ...
            if cached_data:
                ...
                for profile_str, timeframe_list in cached_data.items():
                    profile = SignalProfile(profile_str)
                    result[profile] = [
                        TimeframeConfig(**tf_data) for tf_data in timeframe_list
                    ]
                return result
    ...
    def update_timeframe_configs(
        self, configs: Dict[SignalProfile, List[TimeframeConfig]]
    ) -> bool:
        """
        Update timeframe configurations.

        Args:
            configs: Dictionary mapping signal profiles to timeframe configs
        ...
        try:
            ...
            for profile, timeframe_configs in configs.items():
                serialized_configs[profile.value] = [
                    asdict(tf_config) for tf_config in timeframe_configs
                ]
            ...
            if success:
                logger.info(
                    f"Updated timeframe configurations for {len(configs)} profiles"
                )
                self._invalidate_dependent_caches()
        ...
        try:
            ...
            if cached_data:
                ...
                for matrix_data in cached_data:
                    # Convert signal_profile string back to enum
                    matrix_data["signal_profile"] = SignalProfile(
                        matrix_data["signal_profile"]
                    )
    ...
    def update_strategy_matrix(self, matrix: List[StrategyTimeframeMatrix]) -> bool:
        ...
        try:
            ...
            for item in matrix:
                ...
                item_dict["signal_profile"] = item.signal_profile.value
                matrix_data.append(item_dict)

            success = cache_manager.set(
                self._config_namespace, "strategy_matrix", matrix_data, self._cache_ttl
            )
        ...
        try:
            ...
            for asset in watchlist:
                symbol_config = SymbolStrategyConfig(
                    ...
                    signal_profile=asset.signal_profile,
                )
    ...
    def update_symbol_strategy_config(
        self, symbol: str, config: SymbolStrategyConfig
    ) -> bool:
        """
        ...
        try:
            ...
            for asset in watchlist:
                if asset.symbol == symbol:
                    ...
                    asset.signal_profile = config.signal_profile
                    break
            else:
                logger.warning(f"Symbol {symbol} not found in watchlist")
                return False
    ...
    def update_system_config(self, config: SystemConfig) -> bool:
        ...
        try:
            ...
            success = cache_manager.set(
                self._config_namespace, "system", config_data, self._cache_ttl
            )
            ...
            for asset in watchlist:
                ...
                for strategy_name in asset.enabled_strategies:
                    ...
                    strategy_config = next(
                        (s for s in strategies if s.name == strategy_name), None
                    )
                    if not strategy_config:
                        errors.append(
                            f"Strategy '{strategy_name}' not found for symbol {asset.symbol}"
                        )
                        continue
                    ...
                    if not strategy_config.enabled:
                        warnings.append(
                            f"Strategy '{strategy_name}' is disabled for symbol {asset.symbol}"
                        )
                    ...
                    # Check signal profile compatibility
                    if (
                        matrix_entry
                        and asset.signal_profile != matrix_entry.signal_profile
                    ):
                        warnings.append(
                            f"Signal profile mismatch for symbol {asset.symbol}: "
                            f"asset={asset.signal_profile.value}, "
                            f"strategy={matrix_entry.signal_profile.value}"
                        )
    ...
    def validate_update_intervals(self) -> Dict[str, List[str]]:
        """
        Validate that update intervals are appropriate for each signal profile.
            ...
            for profile, configs in timeframe_configs.items():
                for config in configs:
                    if not config.validate():
                        errors.append(
                            f"Invalid timeframe config for {profile.value}: {config.timeframe}"
                        )

                    # Check reasonable intervals based on profile
                    if profile == SignalProfile.HIGH_FREQUENCY:
                        if (
                            config.timeframe in ["1m", "5m"]
                            and config.update_interval_seconds > 10
                        ):
                            warnings.append(
                                f"Update interval too slow for high frequency signals on {config.timeframe}: "
                                f"{config.update_interval_seconds}s"
                            )
                    elif profile == SignalProfile.MEDIUM_FREQUENCY:
                        if (
                            config.timeframe in ["1h", "4h"]
                            and config.update_interval_seconds > 3600
                        ):
                            warnings.append(
                                f"Update interval too slow for medium frequency signals on {config.timeframe}: "
                                f"{config.update_interval_seconds}s"
                            )
                    elif profile == SignalProfile.LOW_FREQUENCY:
                        if (
                            config.timeframe == "1d"
                            and config.update_interval_seconds > 86400
                        ):
                            warnings.append(
                                f"Update interval too slow for low frequency signals on {config.timeframe}: "
                                f"{config.update_interval_seconds}s"
                            )
    ...
    def get_strategy_for_symbol_and_timeframe(
    ...
    def get_timeframe_config_for_profile(
        self, profile: SignalProfile, timeframe: str
    ) -> Optional[TimeframeConfig]:
        """
        Get timeframe configuration for a specific signal profile and timeframe.
        ...
        try:
            ...
            logger.error(
                f"Failed to get timeframe config for {profile.value}@{timeframe}: {e}"
            )
            return None

    def get_effective_strategy_parameters(
        self, symbol: str, strategy_name: str, timeframe: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to get effective parameters for {symbol}@{strategy_name}@{timeframe}: {e}"
            )
            return {}

    def is_strategy_enabled_for_symbol_timeframe(
        self, symbol: str, strategy_name: str, timeframe: str
    ) -> bool:
        """
        ...
        """
        try:
            enabled_strategies = self.get_strategy_for_symbol_and_timeframe(
                symbol, timeframe
            )
            ...
            logger.error(
                f"Failed to check strategy enablement for {symbol}@{strategy_name}@{timeframe}: {e}"
            )
            return False
    ...
    def get_optimal_timeframe_for_strategy(self, strategy_name: str) -> Optional[str]:
        ...
        try:
            matrix = self.get_strategy_matrix()
            matrix_entry = next(
                (m for m in matrix if m.strategy_name == strategy_name), None
            )
    ...
    def reset_to_defaults(self) -> bool:
        ...
        try:
            ...
            success = (
                self.update_indicator_configs(self._default_indicators)
                and self.update_strategy_configs(self._default_strategies)
                and self.update_system_config(self._default_system_config)
                and self.update_timeframe_configs(self._default_timeframe_configs)
                and self.update_strategy_matrix(self._default_strategy_matrix)
            )
