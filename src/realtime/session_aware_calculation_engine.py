...
from .market_session_manager import MarketSessionManager, SessionState, SessionTransition, MarketType
...
class SessionAwareCalculationEngine:
    ...
    def __init__(self, config_manager: ConfigManager, ingestion_service: MarketDataIngestionService,
        ...
        self.timeframe_manager = EnhancedTimeframeManager(config_manager, self.session_manager)
        ...
        logger.info(f"SessionAwareCalculationEngine initialized for {market_type.value} market")
    ...
    async def initialize(self):
        ...
        try:
            ...
            initial_transition = self.session_manager.update_session_state(datetime.now())
            if initial_transition:
                logger.info(f"Initial session state: {initial_transition.new_state.value}")
    ...
    async def start(self):
        ...
        try:
            ...
            logger.info("SessionAwareCalculationEngine started - continuous operation enabled")
    ...
    async def _process_market_tick_with_session_awareness(self, data: Dict[str, Any]):
        ...
        try:
            ...
            if session_transition:
                await self._handle_session_transition(symbol, timeframes, session_transition)
    ...
    async def _handle_session_transition(self, symbol: str, timeframes: List[str], 
        ...
        try:
            ...
            self.indicator_calculator.handle_session_transition(symbol, session_transition)
            ...
            if session_transition.is_significant_gap:
                logger.warning(f"Significant gap detected for {symbol}: "
    ...
    async def _generate_signals_with_session_context(self, symbol: str, timeframe_results: Dict[str, Any],
        ...
        try:
            ...
            signals = await self._run_signal_generation(symbol, timeframe_results, session_state)
            
            if signals:
                # Filter signals based on session state
                filtered_signals = self._filter_signals_by_session(signals, session_state)
            ...
            logger.error(f"Error generating signals with session context for {symbol}: {e}")
    
    def _filter_signals_by_session(self, signals: List[Dict], session_state: SessionState) -> List[Dict]:
    ...
    async def _store_signals_with_session_context(self, symbol: str, signals: List[Dict],
                    ...
                    **signal,
                    'session_metadata': {
                        ...
                        'session_transition': {
                            ...
                            'gap_duration_hours': session_transition.gap_duration_hours if session_transition else 0,
                            'is_new_trading_day': session_transition.is_new_trading_day if session_transition else False
                        } if session_transition else None,
                        'data_quality_score': self._stats.get('data_quality_score', 100.0)
                    }
                }
                
                # Store in Redis with session-aware key
                signal_key = f"signals:{symbol}:{session_state.value}:{int(time.time())}"
    ...
    def _update_data_quality_score(self, validation_results: List):
        ...
        try:
            ...
            with self._stats_lock:
                ...
                alpha = 0.1
                self._stats['data_quality_score'] = alpha * quality_score + (1 - alpha) * current_score
    ...
    def _calculate_system_health(self):
        ...
        try:
            with self._stats_lock:
                uptime_hours = (datetime.now() - self._stats['continuous_operation_start']).total_seconds() / 3600
                error_rate = self._stats['errors'] / max(1, self._stats['calculations_performed'])
                ...
                health_score = min(100, (
                    ...
                    min(100, uptime_hours) * 0.3 +  # 30% weight on uptime (capped at 100h)
                    max(0, 100 - error_rate * 1000) * 0.3  # 30% weight on error rate
                ))
    ...
    def _get_trading_profile_for_symbol(self, symbol: str) -> TradingProfile:
        """Get trading profile for symbol."""
        # This could be configurable per symbol
        return TradingProfile.SWING_TRADING
    
    def _get_timeframes_for_symbol(self, symbol: str, trading_profile: TradingProfile) -> List[str]:
        """Get timeframes for symbol based on trading profile."""
        if trading_profile == TradingProfile.DAY_TRADING:
            return ["1m", "5m", "15m", "1h"]
        elif trading_profile == TradingProfile.SWING_TRADING:
            return ["5m", "15m", "1h", "1d"]
    ...
    async def _read_market_data_stream(self) -> List[Tuple[str, Dict]]:
        ...
        try:
            ...
            streams = self._redis_client.xread({'market_data_stream': '$'}, count=10, block=100)
    ...
    async def _run_signal_generation(self, symbol: str, timeframe_results: Dict[str, Any], 
                                   session_state: SessionState) -> List[Dict]:
    ...
    def force_session_validation(self, symbol: str) -> str:
        ...
        timeframes = self._get_timeframes_for_symbol(symbol, TradingProfile.SWING_TRADING)
