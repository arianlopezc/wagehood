...
across different timeframes, with signal correlation analysis and composite scoring.
"""
...
from .config_manager import ConfigManager, StrategyConfig, TradingProfile
...
class SignalEngine:
    ...
    to generate sophisticated composite signals with confidence scoring and
    ...
    def __init__(self, config_manager: ConfigManager, timeframe_manager: TimeframeManager):
        ...
        self._signal_history: Dict[str, List[CompositeSignal]] = {}  # {symbol: [signals]}
        self._signal_cache: Dict[str, Dict[str, Dict[str, Signal]]] = {}  # {symbol: {strategy: {timeframe: signal}}}
    ...
    def generate_signals(self, symbol: str, price: float, timeframe_results: Dict[str, Dict[str, Any]],
                        trading_profile: TradingProfile) -> Optional[CompositeSignal]:
        """
        ...
        try:
            with self._lock:
                ...
                for strategy_config in enabled_strategies:
                    ...
                    for timeframe, indicators in timeframe_results.items():
                        ...
                        signal = self._generate_strategy_signal(
                            symbol, strategy_config, timeframe, indicators, price, trading_profile
                        )
                        
                        if signal:
                            individual_signals.append(signal)
                            self._signal_cache[symbol][strategy_config.name][timeframe] = signal
                            self._stats['strategies_processed'].add(strategy_config.name)
                ...
                if individual_signals:
                    composite_signal = self._create_composite_signal(symbol, individual_signals)
    ...
    def _generate_strategy_signal(self, symbol: str, strategy_config: StrategyConfig,
                                timeframe: str, indicators: Dict[str, Any], price: float,
                                trading_profile: TradingProfile) -> Optional[Signal]:
        """
        ...
        try:
            ...
            if strategy_config.name == "macd_rsi_strategy":
                signal_data = self._calculate_macd_rsi_signal(indicators, params, timeframe)
            
            elif strategy_config.name == "ma_crossover_strategy":
                signal_data = self._calculate_ma_crossover_signal(indicators, params, timeframe)
            
            elif strategy_config.name == "rsi_trend_strategy":
                signal_data = self._calculate_rsi_trend_signal(indicators, params, timeframe)
            
            elif strategy_config.name == "bollinger_breakout_strategy":
                signal_data = self._calculate_bollinger_breakout_signal(indicators, params, timeframe, price)
            
            elif strategy_config.name == "sr_breakout_strategy":
                signal_data = self._calculate_sr_breakout_signal(indicators, params, timeframe)
            
            if signal_data:
                direction = SignalDirection(signal_data['direction']) if signal_data['direction'] else SignalDirection.HOLD
                strength = self._calculate_signal_strength(signal_data['confidence'], timeframe)
            ...
            logger.error(f"Error generating {strategy_config.name} signal for {symbol}@{timeframe}: {e}")
            return None
    
    def _calculate_macd_rsi_signal(self, indicators: Dict[str, Any], params: Dict[str, Any], timeframe: str) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_ma_crossover_signal(self, indicators: Dict[str, Any], params: Dict[str, Any], timeframe: str) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_rsi_trend_signal(self, indicators: Dict[str, Any], params: Dict[str, Any], timeframe: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if rsi_value < rsi_oversold:
                ...
                overbought_intensity = (rsi_value - rsi_overbought) / (100 - rsi_overbought)
    ...
    def _calculate_bollinger_breakout_signal(self, indicators: Dict[str, Any], params: Dict[str, Any], 
                                           timeframe: str, price: float) -> Optional[Dict[str, Any]]:
    ...
    def _calculate_sr_breakout_signal(self, indicators: Dict[str, Any], params: Dict[str, Any], timeframe: str) -> Optional[Dict[str, Any]]:
        """Calculate Support/Resistance Breakout strategy signal."""
        try:
            # Simplified S/R implementation using SMA as dynamic S/R level
            ...
            return {
                "direction": None,  # Placeholder - would need price action analysis
                "confidence": 0.5 * tf_multiplier,
                "metadata": {
                    "note": "S/R breakout requires price action context",
                    "sma_50": sma_50,
                    "timeframe_multiplier": tf_multiplier
                }
            }
    ...
    def _calculate_signal_strength(self, confidence: float, timeframe: str) -> SignalStrength:
    ...
    def _create_composite_signal(self, symbol: str, individual_signals: List[Signal]) -> CompositeSignal:
        """
        ...
        try:
            ...
            for tf, votes in timeframe_votes.items():
                ...
                timeframe_confidence[tf] = sum(timeframe_confidences[tf]) / len(timeframe_confidences[tf])
            ...
            for strategy, votes in strategy_votes.items():
                ...
                strategy_confidence[strategy] = sum(strategy_confidences[strategy]) / len(strategy_confidences[strategy])
            ...
            overall_direction = max(direction_scores.items(), key=lambda x: x[1])[0] if direction_scores else SignalDirection.HOLD
            
            # Overall confidence is weighted average
            total_weight = sum(signal.confidence * self._get_timeframe_confidence_multiplier(signal.timeframe) 
                             for signal in individual_signals)
            overall_confidence = total_weight / len(individual_signals) if individual_signals else 0.0
            
            # Calculate signal strength
            overall_strength = self._calculate_composite_signal_strength(individual_signals, overall_confidence)
            
            # Calculate alignment metrics
            timeframe_alignment = self._calculate_timeframe_alignment(timeframe_consensus)
            strategy_alignment = self._calculate_strategy_alignment(strategy_consensus)
            signal_persistence = self._calculate_signal_persistence(symbol, overall_direction)
    ...
    def _calculate_composite_signal_strength(self, signals: List[Signal], overall_confidence: float) -> SignalStrength:
    ...
    def _calculate_timeframe_alignment(self, timeframe_consensus: Dict[str, SignalDirection]) -> float:
    ...
    def _calculate_strategy_alignment(self, strategy_consensus: Dict[str, SignalDirection]) -> float:
    ...
    def _calculate_signal_persistence(self, symbol: str, current_direction: SignalDirection) -> float:
        ...
        try:
            ...
            same_direction_count = sum(1 for signal in recent_signals 
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            stats['total_historical_signals'] = sum(len(history) for history in self._signal_history.values())
        ...
        try:
            ...
            with self._lock:
                for symbol in self._signal_history:
                    ...
                    self._signal_history[symbol] = [
                        signal for signal in self._signal_history[symbol]
