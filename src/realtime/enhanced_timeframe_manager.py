...
class SessionAwareTimeframeState:
    ...
    def should_create_new_candle(self, timestamp: datetime, session_manager: MarketSessionManager, 
                               session_transition: Optional[SessionTransition] = None) -> bool:
        """
        ...
        if session_transition and self._should_create_candle_for_session_transition(session_transition):
            logger.debug(f"Creating new {self.timeframe} candle due to session transition: "
    ...
    def _should_create_candle_for_session_transition(self, session_transition: SessionTransition) -> bool:
        ...
        if self.timeframe == "1d":
            ...
            return (session_transition.is_new_trading_day and 
                   session_transition.transition_time.weekday() == 0)  # Monday
        else:
            # Intraday candles: New candle after significant gaps
            return session_transition.is_significant_gap
    
    def _should_create_candle_for_gap(self, timestamp: datetime, session_manager: MarketSessionManager) -> bool:
        """Check if gap warrants new candle creation."""
        if self.current_candle is None:
            return True
        
        gap_info = session_manager.get_gap_info(self.current_candle.timestamp, timestamp)
    ...
    def _should_create_candle_time_based(self, timestamp: datetime, session_manager: MarketSessionManager) -> bool:
        """Standard time-based candle creation with session alignment."""
        if self.timeframe == "1d":
            # Daily candles align with market sessions, not calendar days
            session_aligned_time = session_manager.get_session_alignment_time(self.timeframe, timestamp)
    ...
    def finalize_current_candle(self, session_transition: Optional[SessionTransition] = None):
    ...
    def handle_session_transition(self, session_transition: SessionTransition):
        ...
        if session_transition.is_significant_gap:
            logger.info(f"Handling significant gap in {self.timeframe}: "
...
class EnhancedTimeframeManager:
    ...
    def __init__(self, config_manager, market_session_manager: Optional[MarketSessionManager] = None):
        """
        ...
        with self._lock:
            ...
            for timeframe in timeframes:
                if timeframe not in self._states[symbol]:
                    ...
                    logger.debug(f"Initialized session-aware timeframe {timeframe} for {symbol}")
        ...
        try:
            ...
            with self._lock:
                ...
                for timeframe in timeframes:
                    ...
                    if tf_state.should_create_new_candle(timestamp, self.session_manager, session_transition):
                        ...
                        logger.debug(f"Created new {timeframe} candle for {symbol} at {timestamp}")
                    else:
    ...
    def _update_timeframe_indicators(self, symbol: str, timeframe: str, price: float, 
                                   ...
                                   session_transition: Optional[SessionTransition] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            gap_detected = tf_state.gap_detected or (session_transition and session_transition.is_significant_gap)
            gap_duration = session_transition.gap_duration_hours if session_transition else 0.0
            ...
            for indicator_config in enabled_indicators:
                try:
                    ...
                    result = self._calculate_indicator_with_gap_awareness(
                        indicator_config, indicator_key, price, tf_state.indicator_calculator,
                        gap_detected, gap_duration
                    )
    ...
    def _calculate_indicator_with_gap_awareness(self, indicator_config, indicator_key: str, 
                                              price: float, calculator: IncrementalIndicatorCalculator,
                                              gap_detected: bool, gap_duration_hours: float):
        ...
        if gap_detected and gap_duration_hours > 16:  # Weekend gap or longer
            if indicator_config.name.startswith("rsi"):
                ...
                logger.debug(f"Reset RSI indicator {indicator_key} due to {gap_duration_hours:.1f}h gap")
        ...
        if indicator_config.name.startswith("sma"):
            ...
            result = calculator.calculate_macd_incremental(indicator_key, price, fast, slow, signal)
            if result:
                return {
                    "macd_line": result[0],
            ...
            result = calculator.calculate_bollinger_bands_incremental(indicator_key, price, period, std_dev)
            if result:
                return {
                    "upper_band": result[0],
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Add gap statistics
            total_gaps = sum(tf_state.gaps_detected 
    ...
    def force_session_update(self, timestamp: Optional[datetime] = None) -> Optional[SessionTransition]:
