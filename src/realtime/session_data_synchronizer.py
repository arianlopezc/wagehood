...
class SessionDataSynchronizer:
    ...
    def __init__(
        ...
        self._validation_history: Dict[str, List[DataValidationResult]] = defaultdict(
            list
        )
    ...
    async def handle_session_transition(
        ...
        for timeframe in timeframes:
            result = await self._validate_session_boundary(
                symbol, timeframe, session_transition
            )
            validation_results.append(result)

            # Schedule additional validation if issues found
            if not result.success:
                await self._schedule_validation_job(
                    symbol, [timeframe], "gap_bridge", priority=1
                )

        # Schedule historical alignment if entering new trading day
        if session_transition.is_new_trading_day:
            await self._schedule_validation_job(
                symbol, timeframes, "historical_sync", priority=2
            )

        return validation_results

    async def _validate_session_boundary(
        self, symbol: str, timeframe: str, session_transition: SessionTransition
        ...
        try:
            ...
            candles = self.timeframe_manager.get_timeframe_data(
            ...
            if len(candles) >= 2:
                gap_issues = self._detect_data_gaps(
                    candles, timeframe, session_transition
                )
            ...
            indicator_issues = await self._validate_indicator_states(
                symbol, timeframe, session_transition
            )
            ...
            logger.error(
                f"Error validating session boundary for {symbol}@{timeframe}: {e}"
            )
    ...
    def _detect_data_gaps(
        self, candles: List, timeframe: str, session_transition: SessionTransition
    ) -> List[str]:
        ...
        for i, candle in enumerate(candles):
            ...
            if i > 0:
                ...
                if price_change > 0.2:  # 20% change
                    issues.append(
                        f"Extreme price movement: {price_change:.1%} in candle {i}"
                    )

        return issues

    async def _validate_indicator_states(
        self, symbol: str, timeframe: str, session_transition: SessionTransition
    ) -> List[str]:
                    ...
                    for sub_name, sub_value in value.items():
                        if isinstance(sub_value, (int, float)):
                            if not (-1e10 < sub_value < 1e10):
                                issues.append(
                                    f"Invalid {indicator_name}.{sub_name} value: {sub_value}"
                                )
    ...
    async def _apply_automatic_corrections(
        self,
        symbol: str,
        timeframe: str,
        issues: List[str],
            ...
            try:
                if "Data gap detected" in issue:
                    ...
                    if hasattr(self.timeframe_manager, "_states"):
                        tf_state = self.timeframe_manager._states.get(symbol, {}).get(
                            timeframe
                        )
    ...
    async def _schedule_validation_job(
        self, symbol: str, timeframes: List[str], job_type: str, priority: int = 5
    ) -> str:
    ...
    async def _sync_with_historical_data(
    ...
    def get_validation_history(
        self, symbol: str, timeframe: str, limit: int = 10
    ) -> List[DataValidationResult]:
        ...
        with self._lock:
            ...
            return {
                **self._stats,
                ...
                "pending_jobs": len(
                    [job for job in self._sync_jobs if job.status == "pending"]
                ),
                "total_jobs_queued": len(self._sync_jobs),
                "average_validation_time_ms": (
                    self._stats["total_validation_time_ms"]
                    / max(1, self._stats["validations_performed"])
                ),
                "validation_success_rate": (
                    (
                        self._stats["validations_performed"]
                        - self._stats["issues_detected"]
                    )
                    / max(1, self._stats["validations_performed"])
                )
                * 100,
    ...
    def force_validation(self, symbol: str, timeframes: List[str]) -> str:
        ...
        return asyncio.create_task(
            self._schedule_validation_job(symbol, timeframes, "validation", priority=0)
        )
