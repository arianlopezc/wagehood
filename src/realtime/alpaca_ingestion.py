...
class AlpacaIngestionService:
    ...
    async def _get_subscription_symbols(self) -> List[str]:
        ...
        try:
            ...
            if not symbols:
                ...
                symbols = [
                    s.strip() for s in default_symbols_str.split(",") if s.strip()
                ]
                logger.warning(
                    f"No symbols configured in watchlist, using environment default: {symbols}"
                )
                logger.warning(
                    "Configure symbols using WATCHLIST_SYMBOLS environment variable"
                )
    ...
    async def _publish_event(self, stream_name: str, event: AlpacaMarketEvent) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Published event {event.event_id} to {stream_name}: {message_id}"
            )
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.running
            and self.connected
            and self.alpaca_provider
            and self.alpaca_provider.is_connected()
...
async def get_alpaca_ingestion_service(
    config_manager: ConfigManager, redis_config: Optional[Dict[str, Any]] = None
) -> AlpacaIngestionService:
    """
