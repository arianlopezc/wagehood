...
class GapAwareIndicatorCalculator(IncrementalIndicatorCalculator):
    ...
    def handle_session_transition(
        self, symbol: str, session_transition: SessionTransition
    ):
        """
    ...
    def _handle_rsi_gap(
        ...
        if gap_duration > self._gap_thresholds["long_gap"]:
            logger.info(
                f"Resetting RSI {indicator_key} for {symbol} due to {gap_duration:.1f}h gap"
            )
            ...
            if hasattr(state, "avg_gain") and hasattr(state, "avg_loss"):
                ...
                logger.debug(
                    f"Applied gap adjustment to RSI {indicator_key} for {symbol}"
                )
    ...
    def _handle_ema_gap(
        ...
        if gap_duration > self._gap_thresholds["weekend_gap"]:
            if hasattr(state, "value"):
                ...
                logger.debug(
                    f"Applied {adjustment_factor} gap adjustment to EMA {indicator_key} for {symbol}"
                )
                self._gap_stats["gap_adjustments_applied"] += 1
    ...
    def _handle_sma_gap(
        ...
        if gap_duration > self._gap_thresholds["holiday_gap"]:
            logger.info(
                f"Resetting SMA {indicator_key} for {symbol} due to {gap_duration:.1f}h gap"
            )
            self.reset_indicator_state(f"{symbol}_{indicator_key}")
            self._gap_stats["indicator_resets"] += 1

    def _handle_macd_gap(
        self,
        symbol: str,
        indicator_key: str,
        state: IndicatorState,
        ...
        if gap_duration > self._gap_thresholds["weekend_gap"] * 1.5:  # 60+ hours
            logger.info(
                f"Resetting MACD {indicator_key} for {symbol} due to {gap_duration:.1f}h gap"
            )
            self.reset_indicator_state(f"{symbol}_{indicator_key}")
            self._gap_stats["indicator_resets"] += 1
    ...
    def calculate_sma_incremental_with_gap_awareness(
        self,
        symbol: str,
        price: float,
        period: int,
    ...
    def calculate_ema_incremental_with_gap_awareness(
        self,
        symbol: str,
        price: float,
        period: int,
        ...
        if gap_info and gap_info.get("is_significant", False):
            ...
            if (
                ...
                and state_key in self._states[symbol]
                and gap_duration > self._gap_thresholds["weekend_gap"]
            ):
    ...
    def calculate_rsi_incremental_with_gap_awareness(
        self,
        symbol: str,
        price: float,
        period: int,
        ...
        if gap_info and gap_info.get("is_significant", False):
            ...
            if gap_duration > self._gap_thresholds["long_gap"]:
                ...
                if symbol in self._states and state_key in self._states[symbol]:
                    ...
                    self._gap_stats["indicator_resets"] += 1
                    logger.debug(
                        f"Reset RSI for {symbol} due to {gap_duration:.1f}h gap"
                    )
    ...
    def get_all_indicators_with_gap_info(self, symbol: str) -> Dict[str, Any]:
        ...
        gap_info = {"gap_stats": self.get_gap_stats(), "indicators": base_indicators}
