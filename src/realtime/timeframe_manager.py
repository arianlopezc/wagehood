...
class TimeframeState:
    ...
    def should_create_new_candle(self, timestamp: datetime) -> bool:
        ...
        current_interval_start = (
            int(self.current_candle.timestamp.timestamp()) // self.base_interval_seconds
        ) * self.base_interval_seconds
...
class TimeframeManager:
    ...
    def __init__(self, config_manager):
        ...
        self._states: Dict[str, Dict[str, TimeframeState]] = (
            {}
    ...
    def _update_timeframe_indicators(
        self, symbol: str, timeframe: str, price: float, tf_state: TimeframeState
    ) -> Dict[str, Any]:
        """
            ...
            for indicator_config in enabled_indicators:
                try:
                    ...
                    if indicator_config.name.startswith("sma"):
                        ...
                        result = (
                            tf_state.indicator_calculator.calculate_sma_incremental(
                        ...
                        period = indicator_config.parameters.get("period", 50)
                        result = (
                            tf_state.indicator_calculator.calculate_ema_incremental(
                                indicator_key, price, period
                            )
                        )
                        if result is not None:
                        ...
                        result = (
                            tf_state.indicator_calculator.calculate_rsi_incremental(
                                indicator_key, price, period
                            )
                        )
                        if result is not None:
                        ...
                        result = (
                            tf_state.indicator_calculator.calculate_macd_incremental(
    ...
    def get_cross_timeframe_correlation(
            ...
            if correlation_data:
                ...
                if "rsi_14" in common_indicators:
                    ...
                    analysis["trend_alignment"]["rsi"] = {
                        ...
                        "conflicting": len(set(v > 50 for v in rsi_values.values()))
                        > 1,
                    }
            ...
            with self._lock:
                for symbol in self._states:
                    for timeframe in self._states[symbol]:
                        ...
                        while (
                            tf_state.candles
                            and tf_state.candles[0].timestamp < cutoff_time
                        ):
            ...
            with self._lock:
                ...
                for symbol, tf_states in self._states.items():
                    ...
                    for timeframe, tf_state in tf_states.items():
                        ...
                        symbol_data["memory_estimate_mb"] += (
                            candle_count * 50 / (1024 * 1024)
                        )
