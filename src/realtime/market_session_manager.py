...
class MarketSessionManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized MarketSessionManager for {market_type.value} in {timezone}"
        )
    ...
    def update_session_state(self, timestamp: datetime) -> Optional[SessionTransition]:
        ...
        with self._lock:
            ...
            if new_state != self._current_state or self._last_tick_time is None:
                ...
                logger.info(
                    f"Session transition: {self._current_state.value} → {new_state.value} "
    ...
    def should_handle_gap(
        self, last_timestamp: datetime, current_timestamp: datetime
    ) -> bool:
        """
    ...
    def get_gap_info(
        ...
        return {
            ...
            "is_weekend_gap": self._is_weekend_gap(gap_duration),
            "crosses_session_boundary": self._crosses_session_boundary(
                last_timestamp, current_timestamp
            ),
            "gap_type": self._classify_gap(gap_duration),
            "last_session_state": self._determine_session_state(
                self._to_market_time(last_timestamp)
            ),
            "current_session_state": self._determine_session_state(
                self._to_market_time(current_timestamp)
            ),
        }

    def get_session_alignment_time(
        self, timeframe: str, timestamp: datetime
    ) -> datetime:
        """
    ...
    def _determine_session_state(self, market_time: datetime) -> SessionState:
        ...
        if self.market_type == MarketType.CRYPTO:
            ...
            if (
                self._schedule["regular_open"]
                <= current_time
                < self._schedule["regular_close"]
            ):
