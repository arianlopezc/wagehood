...
from ..core.models import BacktestResult, PerformanceMetrics
...
class StrategyComparator:
    ...
    def _create_comparison_summary(self, results: List[BacktestResult]) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'worst_return': min(r.performance_metrics.total_return_pct for r in results),
    ...
    def _create_detailed_comparison(self, results: List[BacktestResult]) -> Dict[str, Dict[str, Any]]:
        ...
        for result in results:
            ...
            detailed[result.strategy_name] = {
                ...
                'risk_metrics': {
                    ...
                    'calmar_ratio': self.evaluator.calculate_calmar_ratio(result.equity_curve),
                    'var_5pct': self.evaluator.calculate_var(result.equity_curve, 0.05),
                    'expected_shortfall': self.evaluator.calculate_expected_shortfall(result.equity_curve)
                },
                'trading_metrics': {
                ...
                'consistency_metrics': {
                    ...
                    'return_volatility': self._calculate_return_volatility(result.equity_curve)
                }
            }
        
        return detailed
    
    def _create_rankings(self, results: List[BacktestResult]) -> Dict[str, List[Tuple[str, float]]]:
    ...
    def _create_risk_analysis(self, results: List[BacktestResult]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            risk_analysis['tail_risk_analysis'][result.strategy_name] = {
                ...
                'var_5pct': self.evaluator.calculate_var(result.equity_curve, 0.05),
                'expected_shortfall_5pct': self.evaluator.calculate_expected_shortfall(result.equity_curve, 0.05),
    ...
    def _create_correlation_analysis(self, results: List[BacktestResult]) -> Dict[str, Any]:
    ...
    def _create_recommendations(self, results: List[BacktestResult]) -> List[Dict[str, Any]]:
        ...
        # Most consistent
        lowest_drawdown = min(results, key=lambda x: x.performance_metrics.max_drawdown_pct)
        ...
        highest_return = max(results, key=lambda x: x.performance_metrics.total_return_pct)
    ...
    def create_comparison_report(self, results: List[BacktestResult]) -> str:
        ...
        report.append(f"Date Range: {summary['date_range']['start'].strftime('%Y-%m-%d')} to {summary['date_range']['end'].strftime('%Y-%m-%d')}")
    ...
    def _calculate_drawdown_periods(self, equity_curve: List[float]) -> List[Dict[str, Any]]:
            ...
            if value > peak:
                if in_drawdown:
                    ...
                    drawdown_periods.append({
                        ...
                        'depth': (peak - min(equity_curve[drawdown_start:i])) / peak
                    })
                    in_drawdown = False
                peak = value
            else:
    ...
    def _analyze_diversification_benefits(self, results: List[BacktestResult]) -> Dict[str, Any]:
        ...
        for result in results:
            ...
            if returns:
                ...
                individual_volatilities.append(statistics.stdev(returns) if len(returns) > 1 else 0)
        ...
        if individual_returns:
            ...
            return {
                ...
                'diversification_ratio': avg_volatility / (avg_volatility * 0.8)  # Simplified
            }
        
        return {}
    
    def _suggest_portfolio_combinations(self, results: List[BacktestResult]) -> List[Dict[str, Any]]:
            ...
            for j, result2 in enumerate(results[i+1:], i+1):
                ...
                if corr < 0.3:  # Low correlation
                    suggestions.append({
                        ...
                        'strategies': [result1.strategy_name, result2.strategy_name],
                        'correlation': corr,
