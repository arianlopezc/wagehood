...
def calculate_bollinger_bands(data: Union[np.ndarray, list], period: int = BB_PERIOD, 
    ...
    if len(data_array) < period:
        raise ValueError(f"Insufficient data: need {period} points, got {len(data_array)}")
...
def calculate_atr(high: Union[np.ndarray, list], low: Union[np.ndarray, list], 
    ...
    if len(close_array) < period + 1:
        raise ValueError(f"Insufficient data: need {period + 1} points, got {len(close_array)}")
...
def calculate_keltner_channels(high: Union[np.ndarray, list], low: Union[np.ndarray, list], 
                              close: Union[np.ndarray, list], period: int = 20, 
...
def calculate_donchian_channels(high: Union[np.ndarray, list], low: Union[np.ndarray, list], 
    ...
    if len(high_array) < period:
        raise ValueError(f"Insufficient data: need {period} points, got {len(high_array)}")
...
def calculate_volatility(data: Union[np.ndarray, list], period: int = 20, 
                        annualize: bool = False, trading_periods: int = 252) -> np.ndarray:
    """
...
def calculate_bb_width(upper_band: np.ndarray, lower_band: np.ndarray, 
                      middle_band: np.ndarray) -> np.ndarray:
    """
    ...
    # Calculate BB Width
    bb_width = np.where(middle_band != 0, (upper_band - lower_band) / middle_band, np.nan)
    
    return bb_width


def calculate_bb_percent(data: Union[np.ndarray, list], upper_band: np.ndarray, 
                        lower_band: np.ndarray) -> np.ndarray:
    """
...
def calculate_price_channels(high: Union[np.ndarray, list], low: Union[np.ndarray, list], 
