...
def calculate_support_resistance(data: Union[np.ndarray, list], lookback: int = 20, 
                                min_touches: int = 3, tolerance: float = 0.001) -> Dict[str, List[float]]:
    """
    ...
    if len(data_array) < lookback * 2:
        raise ValueError(f"Insufficient data: need at least {lookback * 2} points, got {len(data_array)}")
    ...
    resistance_levels = _group_and_count_levels(resistance_candidates, tolerance, min_touches)
...
def _group_and_count_levels(levels: np.ndarray, tolerance: float, min_touches: int) -> List[float]:
    """
    ...
    for i in range(1, len(sorted_levels)):
        ...
        if abs(sorted_levels[i] - np.mean(current_group)) / np.mean(current_group) <= tolerance:
...
def calculate_pivot_points(high: Union[np.ndarray, list], low: Union[np.ndarray, list], 
                          close: Union[np.ndarray, list], method: str = 'standard') -> Dict[str, float]:
    """
...
def _calculate_standard_pivots(high: float, low: float, close: float) -> Dict[str, float]:
...
def _calculate_fibonacci_pivots(high: float, low: float, close: float) -> Dict[str, float]:
...
def _calculate_camarilla_pivots(high: float, low: float, close: float) -> Dict[str, float]:
...
def calculate_fibonacci_extensions(high: float, low: float, retrace: float,
                                  trend: str = 'up') -> Dict[str, float]:
    """
...
def detect_breakouts(data: Union[np.ndarray, list], levels: List[float], 
                     tolerance: float = 0.001, min_volume_ratio: float = 1.5,
                     volume: Optional[Union[np.ndarray, list]] = None) -> List[Dict]:
    """
...
def calculate_trend_lines(data: Union[np.ndarray, list], 
                         window: int = 20, 
                         min_points: int = 3) -> Dict[str, List[Dict]]:
    """
    ...
    if len(data_array) < window:
        raise ValueError(f"Insufficient data: need {window} points, got {len(data_array)}")
    ...
    for i in range(len(local_minima_idx) - min_points + 1):
        ...
        if slope > 0:
            uptrend_lines.append({
                ...
                'r_squared': _calculate_r_squared(points_idx, points_price, slope, intercept)
            })
    ...
    return {
        'uptrend': uptrend_lines,
        'downtrend': downtrend_lines
    }


def _calculate_r_squared(x: np.ndarray, y: np.ndarray, slope: float, intercept: float) -> float:
