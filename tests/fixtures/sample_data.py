...
class SampleDataProvider:
        ...
        for daily_bar in daily_data[:10]:  # Just first 10 days
            for hour in range(24):
                if 9 <= hour <= 16:  # Market hours only
                    timestamp = daily_bar.timestamp.replace(hour=hour, minute=0, second=0)
    ...
    def get_known_indicator_test_data(self) -> Dict[str, Any]:
        ...
        expected_indicators = {
            "sma_5": {
                "values": [None, None, None, None, 102.8, 104.2, 105.0, 105.8, 107.2, 108.0, 108.8, 110.2, 111.0, 112.0, 113.2],
                "description": "5-period Simple Moving Average"
            },
            "ema_5": {
                "values": [None, None, None, None, 102.8, 104.37, 105.25, 106.5, 108.0, 108.33, 109.55, 111.03, 111.69, 112.79, 114.19],
                "description": "5-period Exponential Moving Average"
            },
            "rsi_5": {
                "values": [None] * 5 + [75.0, 60.0, 70.0, 80.0, 65.0, 75.0, 85.0, 70.0, 80.0, 90.0],
