...
class ComprehensiveTestRunner:
    ...
    def _configure_test_suites(self) -> List[TestSuite]:
        ...
        base_suites = [
            ...
            TestSuite(
                ...
                dependencies=["mathematical", "integration"] if not self.args.quick else [],
                parallel=True,
                timeout=self.args.timeout,
                environment=self.args.environment,
            ...
            TestSuite(
                ...
                dependencies=["mathematical", "integration", "workers"] if not self.args.quick else [],
                parallel=False,
                timeout=self.args.timeout,
                environment=self.args.environment,
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.args.suite:
                # Run specific suite
                suite = next((s for s in self.test_suites if s.name == self.args.suite), None)
                if suite:
            ...
            self.logger.info(f"Comprehensive test execution completed in {total_time:.2f}s")
    ...
    def _generate_reports(self):
        ...
        html_report = self.report_generator.generate_comprehensive_report(
            self.results,
            output_path=str(self.report_dir / "comprehensive_report.html")
        )
        ...
        json_report = self.report_generator.generate_json_report(
            self.results,
            output_path=str(self.report_dir / "test_results.json")
        )
        ...
        summary_report = self.report_generator.generate_summary_report(
            self.results,
            output_path=str(self.report_dir / "test_summary.txt")
        )
    ...
    def _calculate_summary(self) -> Dict[str, Any]:
        ...
        error_suites = sum(1 for r in self.results.values() if r.status == TestStatus.ERROR)
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--ci",
        action="store_true",
        help="Run in CI mode (optimized for CI/CD)"
    )
        ...
        if "summary" in results:
            ...
            if "results" in results:
                ...
                for suite_name, result in results["results"].items():
                    ...
                    print(f"{status_symbol} {suite_name}: {result.status.value} ({result.duration:.2f}s)")
