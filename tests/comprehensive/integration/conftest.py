...
def performance_requirements():
    ...
    return {
        "latency": {"avg_ms": 5.0, "p95_ms": 20.0, "p99_ms": 50.0, "max_ms": 100.0},
        "throughput": {"min_ops_per_sec": 500, "target_ops_per_sec": 1000},
        "reliability": {"max_error_rate": 0.01, "min_uptime": 0.99},
        "resources": {"max_memory_mb": 50, "max_cpu_percent": 80},
    }
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "alpaca: mark test as requiring Alpaca API")
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_integration = pytest.mark.skip(
        reason="Integration tests require external dependencies"
    )
