...
from src.realtime.config_manager import ConfigManager, TradingProfile, AssetConfig, SystemConfig
...
def test_config_manager():
    ...
    with patch('src.realtime.config_manager.cache_manager'):
        ...
        config_manager.get_enabled_indicators = Mock(return_value=config_manager._default_indicators)
        config_manager.get_enabled_strategies = Mock(return_value=config_manager._default_strategies)
        config_manager.get_system_config = Mock(return_value=SystemConfig())
        config_manager.get_timeframe_configs = Mock(return_value=config_manager._default_timeframe_configs)
        config_manager.get_strategy_matrix = Mock(return_value=config_manager._default_strategy_matrix)
...
@pytest.fixture
async def test_calculation_engine(test_config_manager, test_data_ingestion_service, mock_redis_client):
...
def load_generator():
    ...
    class LoadGenerator:
        ...
        def generate_message_load(self, target_function, messages_per_second=10, duration=5):
    ...
    class MessageFactory:
        ...
        def create_calculation_event(self, symbol="AAPL", results=None):
            ...
            return {
                ...
                "symbol": symbol,
                "timestamp": datetime.now().isoformat(),
                "results": json.dumps(results or {"test": True, "id": self.message_counter})
            }
            
        def create_alert_message(self, alert_type="signal", symbol="AAPL", message="Test alert"):
