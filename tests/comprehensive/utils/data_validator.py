...
class DataValidator:
    ...
    def validate_indicator_properties(
        ...
        try:
            ...
            if min_value is not None and indicator_values.min() < min_value:
                issues.append(f"Minimum value {indicator_values.min()} below expected {min_value}")
            
            if max_value is not None and indicator_values.max() > max_value:
                issues.append(f"Maximum value {indicator_values.max()} above expected {max_value}")
            
            # Check for bounded indicators (like RSI, Stochastic)
            if should_be_bounded:
                if min_value is not None and max_value is not None:
                    out_of_bounds = (
                        (indicator_values < min_value) | 
                        (indicator_values > max_value)
                    ).sum()
                    if out_of_bounds > 0:
                        issues.append(f"{out_of_bounds} values out of bounds [{min_value}, {max_value}]")
            ...
            if len(indicator_values) > 0:
                ...
                if std_val > 0:
                    ...
                    if outlier_count > len(indicator_values) * 0.05:  # More than 5% outliers
            ...
            message = f"Indicator properties validation {'passed' if passed else 'failed'}"
    ...
    def validate_trading_signals(
        ...
        try:
            ...
            if not valid_signals.all():
                ...
                issues.append(f"{invalid_count} invalid signal values (not in [-1, 0, 1])")
            ...
            if expected_signal_count is not None:
                ...
                if abs(actual_count - expected_signal_count) > 0.1 * expected_signal_count:
                    issues.append(f"Signal count {actual_count} differs from expected {expected_signal_count}")
    ...
    def validate_portfolio_calculations(
        ...
        try:
            ...
            details['initial_value'] = portfolio_values.iloc[0] if len(portfolio_values) > 0 else 0
            details['final_value'] = portfolio_values.iloc[-1] if len(portfolio_values) > 0 else 0
            details['total_return'] = (details['final_value'] / details['initial_value'] - 1) * 100
            ...
            if len(portfolio_values) > 0:
                ...
                if abs(actual_initial - initial_capital) > 0.01:
                    issues.append(f"Initial capital {actual_initial} differs from expected {initial_capital}")
            ...
            if len(portfolio_values) > 1 and len(returns) > 0:
                ...
                if len(calculated_returns) == len(returns):
                    ...
                    if max_diff > 0.001:  # 0.1% tolerance
                        issues.append(f"Return calculation inconsistency: max diff {max_diff:.4f}")
            ...
            passed = len(issues) == 0
            severity = ValidationSeverity.WARNING if not passed else ValidationSeverity.INFO
            message = f"Portfolio calculations validation {'passed' if passed else 'failed'}"
