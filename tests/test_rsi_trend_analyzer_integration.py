...
class TestRSITrendAnalyzerIntegration:
    ...
    async def test_signal_chronological_order(self, rsi_analyzer, test_symbols):
        """Test that generated signals are in chronological order."""
        symbol = test_symbols[0]
        end_date = datetime.now()
        ...
        # If we have multiple signals, verify they're in chronological order
        if len(signals) > 1:
            timestamps = [signal['timestamp'] for signal in signals]
            assert timestamps == sorted(timestamps), "Signals should be in chronological order"
