#!/usr/bin/env python3
"""
Cron Wrapper for 1-Hour Analysis Trigger

This script manages the execution of the 1-hour analysis trigger with:
...
# Configuration
TIMEOUT_SECONDS = 30
SCRIPT_NAME = "trigger_1h_analysis.py"
LOCK_FILE = Path.home() / '.wagehood' / 'cron_1h.lock'
PID_FILE = Path.home() / '.wagehood' / 'cron_1h.pid'
LOG_FILE = Path.home() / '.wagehood' / 'cron_1h.log'
TIMEOUT_TRACKING_FILE = Path.home() / '.wagehood' / 'cron_1h_timeout_tracking.json'
...
class CronJobManager:
    ...
    async def send_infrastructure_alert(self, message: str):
        ...
        try:
            notification = NotificationMessage.create_service_notification(
                content=f"🚨 **1-Hour Analysis Cron Job Issue**\n\n{message}\n\n⏰ Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    def run_with_timeout(self) -> bool:
        ...
        logger.info(f"Starting 1-hour analysis at {datetime.now()}")
        ...
        try:
                ...
                if elapsed > TIMEOUT_SECONDS:
                    ...
                    if should_alert:
                        ...
                        asyncio.run(self.send_infrastructure_alert(
                            f"⚠️ REPEATED TIMEOUTS: 1-hour analysis has timed out {self.timeout_tracking['consecutive_timeouts']} times in a row.\n\n"
            ...
            # Log results
            if return_code == 0:
                logger.info(f"1-hour analysis completed successfully in {execution_time:.1f}s")
                # Reset timeout tracking on successful run
                self._reset_timeout_tracking()
            else:
                logger.error(f"1-hour analysis failed with code {return_code} after {execution_time:.1f}s")
                
                # Send alert for persistent failures
                if execution_time > 5:  # Only alert for non-quick failures
                    import asyncio
                    asyncio.run(self.send_infrastructure_alert(
                        f"1-hour analysis failed with exit code {return_code}.\n"
            ...
            import asyncio
            asyncio.run(self.send_infrastructure_alert(
                f"Failed to execute 1-hour analysis script.\n"
                f"Error: {str(e)}"
            ))
            return False
